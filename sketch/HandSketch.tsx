...
export const HandSketch = ({ handpose }: Props) => {
  ...
  const draw = (p5: p5Types) => {
    ...
    p5.push();
    p5.translate(p5.width / 2 - 200, p5.height / 2);
    ...
    for (let k = 0; k < 30; k++) {
      ...
      if (smoothedHandposeHistory.left[index].length !== 0) {
        ...
        for (let i = 0; i < 4; i++) {
          p5.ellipse(
            left_hand[i + 4].x - left_hand[0].x,
            left_hand[i + 4].y - left_hand[0].y,
            10
          );
        }
      }
    ...
    for (let k = 0; k < 30; k++) {
      ...
      p5.rotate((k * 2 * Math.PI) / 30);
      const index = k * 2;
      if (smoothedHandposeHistory.right[index].length !== 0) {
        const right_hand = smoothedHandposeHistory.right[index];
        for (let i = 0; i < 4; i++) {
          p5.ellipse(
            right_hand[i + 4].x - right_hand[0].x,
            right_hand[i + 4].y - right_hand[0].y,
            10
          );
        }
      }
